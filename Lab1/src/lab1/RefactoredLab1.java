...
public class RefactoredLab1 {
	// ALREADY IMPLEMENTED; DO NOT MODIFY

	private RefactoredLab1() {
  ...
  * 
   ...
   * </p>
	...
	public static String weather(int month, int day) 
	{
		...
		// REFACTOR USING ARRAYS, OTHERWISE NOT IMMEDIATELY OBVIOUS HOW, MAYBE A SWITCH FUNCTION
	...
	public static String temperatureConverter(double fahrenheit) 
	{
		...
		// NOTHING OBVIOUS TO REFACTOR HERE
		...
		// all values in calculation must have the same type, otherwise yields incorrect result (rather, at least one term involved in operation)
	...
	public static String fiveYearBalance(double balance, double interest) {
		...
		// storing evaluations in variables, clarifies branching, added decimals since arguments passed are double data types (not necessary)
		boolean invalidBalance = balance < 0.0;
		boolean invalidInterest = interest < 0.0 || interest > 100.0;
		...
		for (int i = 1; i <= 5; i++) 
		{
			// shorthand formula for compound interest, added decimals to make all values doubles
			new_balance *= (1.0 + interest / 100.0);
		}
	...
	public static double computeBMI(int pounds, int feet, int inches) {
		...
		// converting from imperial to metric units, is multiplication between double and int implicitly converting int to double?
  ...
  * </p>
	...
	public static String isPerfectSquare(int n) {
		...
		// checking if square root of a value ends with .0 (is an integer), is sqrt method converting int to double since parameter is double type?
